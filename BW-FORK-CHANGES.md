...
## INTRATE Financial Formula

- `base/src/functions/mod.rs`
  - Registers the English `INTRATE` function name for Conductor financial
    formula evidence without changing localized language payloads.
- `base/src/functions/financial.rs`
  - Adds scalar `INTRATE(settlement, maturity, investment, redemption, [basis])`
    evaluation using the existing Excel date-basis year fraction helper.
- `base/src/expressions/parser/static_analysis.rs`
  - Classifies `INTRATE` as scalar-only input with optional basis.

## RECEIVED Financial Formula

- `base/src/functions/mod.rs`
  - Registers the English `RECEIVED` function name for Conductor financial
    formula evidence without changing localized language payloads.
- `base/src/functions/financial.rs`
  - Adds scalar `RECEIVED(settlement, maturity, investment, discount, [basis])`
    evaluation using the existing Excel date-basis year fraction helper.
- `base/src/expressions/parser/static_analysis.rs`
...
## ACCRINTM Financial Formula

- `base/src/functions/mod.rs`
  - Registers the English `ACCRINTM` function name for Conductor financial
    formula evidence without changing localized language payloads.
- `base/src/functions/financial.rs`
  - Adds scalar `ACCRINTM(issue, maturity, rate, par, [basis])` evaluation
    using the existing Excel date-basis year fraction helper.
- `base/src/expressions/parser/static_analysis.rs`
  - Classifies `ACCRINTM` as scalar-only input with optional basis.

## AMORDEGRC Financial Formula

- `base/src/functions/mod.rs`
  - Registers the English `AMORDEGRC` function name for Conductor financial
    formula evidence without changing localized language payloads.
- `base/src/functions/financial.rs`
  - Adds scalar `AMORDEGRC(cost, date_purchased, first_period, salvage,
    period, rate, [basis])` evaluation using the existing Excel date-basis
    year fraction helper and degressive depreciation coefficients.
- `base/src/expressions/parser/static_analysis.rs`
...
## COUPDAYS Financial Formula

- `base/src/functions/mod.rs`
  - Registers the English `COUPDAYS` function name for Conductor financial
    formula evidence without changing localized language payloads.
- `base/src/functions/financial.rs`
  - Adds scalar `COUPDAYS(settlement, maturity, frequency, [basis])`
    evaluation for the length of the settlement coupon period, using fixed
    30/360 and actual/360 or actual/365 period lengths where Excel defines
    those bases and actual coupon-date spans for basis 1.
- `base/src/expressions/parser/static_analysis.rs`
...
## COUPNCD Financial Formula

- `base/src/functions/mod.rs`
  - Registers the English `COUPNCD` function name for Conductor financial
    formula evidence without changing localized language payloads.
- `base/src/functions/financial.rs`
  - Adds scalar `COUPNCD(settlement, maturity, frequency, [basis])`
    evaluation for the serial date of the next coupon after settlement.
- `base/src/expressions/parser/static_analysis.rs`
