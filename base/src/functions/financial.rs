...
fn next_coupon_date(
    ...
    let mut coupon = maturity;
    while coupon > settlement {
        let previous = shift_months_clamped(coupon, -months_per_coupon)?;
...
impl<'a> Model<'a> {
    ...
    pub(crate) fn fn_coupdays(&mut self, args: &[Node], cell: CellReferenceIndex) -> CalcResult {
        ...
        let result = match basis {
            ...
            1 => {
                ...
                (next_coupon - previous_coupon).num_days() as f64
            }
            _ => unreachable!(),
        };
        ...
        if result.is_nan() {
            return CalcResult::new_error(
                Error::NUM,
                cell,
                "Invalid data for COUPDAYS".to_string(),
            );
        }
        CalcResult::Number(result)
    }
    ...
    pub(crate) fn fn_coupdaysnc(&mut self, args: &[Node], cell: CellReferenceIndex) -> CalcResult {
        ...
        let result = match basis {
            0 | 4 => {
                let next_coupon_serial = match date_to_serial_number(
                    next_coupon.day(),
                    next_coupon.month(),
                    next_coupon.year(),
                ) {
                ...
                match self.fn_days360(
                    &[
                        Node::NumberKind(settlement_serial as f64),
                        Node::NumberKind(next_coupon_serial as f64),
                        Node::NumberKind(method),
                    ],
                    cell,
                ) {
            ...
            1..=3 => (next_coupon - settlement).num_days() as f64,
            _ => unreachable!(),
        };
        ...
        if result.is_nan() {
            return CalcResult::new_error(
                Error::NUM,
                cell,
                "Invalid data for COUPDAYSNC".to_string(),
            );
        }
        CalcResult::Number(result)
    }
    ...
    pub(crate) fn fn_coupnum(&mut self, args: &[Node], cell: CellReferenceIndex) -> CalcResult {
        ...
        while coupon > settlement {
            coupons += 1;
            coupon = match shift_months_clamped(coupon, -months_per_coupon) {
                Some(date) => date,
                None => return CalcResult::new_error(Error::NUM, cell, "Invalid date".to_string()),
            };
        }
    ...
    pub(crate) fn fn_intrate(&mut self, args: &[Node], cell: CellReferenceIndex) -> CalcResult {
        ...
        let result = ((redemption - investment) / investment) / year_fraction;
        if result.is_infinite() {
            return CalcResult::new_error(Error::DIV, cell, "Division by 0".to_string());
        }
        if result.is_nan() {
            return CalcResult::new_error(Error::NUM, cell, "Invalid data for INTRATE".to_string());
        }
    ...
    pub(crate) fn fn_received(&mut self, args: &[Node], cell: CellReferenceIndex) -> CalcResult {
        ...
        let result = investment / (1.0 - discount * year_fraction);
        if result.is_infinite() {
            return CalcResult::new_error(Error::DIV, cell, "Division by 0".to_string());
        }
        if result.is_nan() {
            return CalcResult::new_error(
                Error::NUM,
                cell,
                "Invalid data for RECEIVED".to_string(),
            );
        }
